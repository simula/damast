...
# This radius is optimized to keep global errors low on average.
EARTH_RADIUS_IN_KM: float = 6371.0  # km

# Using an Earth radius of 6,373 km is a specific optimization used in geodesy and navigation to minimize errors when calculating distances across mid-latitude regions, such as the United States, Europe, and Asia.
# Use EARTH_RADIUS_MID_LATITUDE_IN_KM = 6,373 km when you want a quick, spherical distance calculation that is highly accurate for mid-latitude geography,
EARTH_RADIUS_MID_LATITUDE_IN_KM: float = 6373.0
...
def reverse_bearing(lat_1: npt.NDArray[np.float64],
                    ...
                    earth_radius_in_km: float = EARTH_RADIUS_IN_KM) -> npt.NDArray[np.float64]:
    """
...
"""
    ...
    ad = distance / earth_radius_in_km
...
def chord_distance(d: npt.NDArray[np.float64], earth_radius_in_km: float = EARTH_RADIUS_IN_KM) -> npt.NDArray[np.float64]:
    ...
    """
    return 2 * earth_radius_in_km * np.sin(d / (2 * earth_radius_in_km))


def angle_sat_c(c: npt.NDArray[np.float64], earth_radius_in_km: float = EARTH_RADIUS_IN_KM) -> npt.NDArray[np.float64]:
    """
    ...
    """
    return np.pi - np.arccos(c / (2 * earth_radius_in_km))
